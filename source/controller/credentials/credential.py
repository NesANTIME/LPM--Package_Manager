...
from source.modules.chargate_config import (
    returnLocal_FileSources, returnLocal_RutaLPM, 
    returnLocal_RutaPATH, returnLocal_RutaPackagesLPM
)
...
def create_credentials(file_path): 
    ...
    keyring.set_password(huella_system, huella_user, password_account)
...
def verify_userConfig():
    ...
    token = keyring.get_password(huella_system, huella_user)
    id_cliend, token_client = split_at_symbol(token)

    return id_cliend, token_client